...
In this case we are sending to the shader a texture named `texture_1` with the content of the `fbo` number 2, everything loaded in texture unit 0.# Engine Usage keys
## Control keys
* Exit Demo : ESC
...
`uniform float scale v1:=1+0.25*beat0;`

`uniform vec3 color v1:=1.0;v2:=1+5.0*beat0;v3:=1.0;`



## Considerations when writing formulas:

* Equal (=) is written like this: `:=`, for example: `uniform float scale v1:=10*beat0;`
* At the end of any formula, we should write a semicolon `;`, for example: `uniform float scale v1:=10*beat0;`
* If you want to write multiple formulas, split them also with semicolon, for example: `uniform vec3 rotation v1:=90*sin(t); v2:=0; v3:=180*cos(t);`
* A complete description of the formulas you can make can be found here: https://archive.codeplex.com/?p=fastmathparser
...
`uniform float scale v1:=1+0.1*beat0;`
