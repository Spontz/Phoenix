...
void main()
{
	...
	if (gs_in[0].Type == PARTICLE_TYPE_EMITTER) {
		...
		if (Age<gs_in[0].Life) {
			...
			//o_Position = gs_in[0].InitPosition + (vec3(1,0,0) - gs_in[0].InitPosition) * AgeZtoO + gs_in[0].Rotation*gDeltaTime; // Put your random formula here
		}
		// If particle has died, then we need to restart it
			...
			o_Age = 0;
			o_Position = gs_in[0].InitPosition;
			//o_Rotation = GetRandomDir(gTime+gs_in[0].ID);
			o_Rotation = GetRandomDir(gTime+fParticleID);
			o_Color = vec3(1,0,0);//GetRandomDir(gTime+gs_in[0].ID);
		}
