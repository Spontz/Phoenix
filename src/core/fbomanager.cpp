...
int FboManager::addFbo(string engine_format, int width, int height, int iformat, int format, int type, int components) {
	int fbo_id = -1;
	Fbo *new_fbo = new Fbo();
	if (engine_format != "DEPTH") { // If it's and FBO with Color attachment
		if (new_fbo->uploadColor((int)fbo.size(), width, height, iformat, format, type)) {
	...
	else {	// If it's not an FBO with Color attachment (depth only)
		if (new_fbo->uploadDepth((int)fbo.size(), width, height, iformat, format, type)) {
			fbo.push_back(new_fbo);
			mem += (float)(new_fbo->width * new_fbo->height * components) / 1048576.0f;		// increase the texture mem
			fbo_id = (int)fbo.size() - 1;
			LOG->Info(LOG_MED, "Fbo Depth %d loaded OK. Overall fbo Memory: %.3fMb", fbo_id, mem);
		}
		else {
			LOG->Error("Could not load fbo with the current format: Width: %d, Height: %d, iformat: %d, format: %d, type: %d", width, height, iformat, format, type);
			return -1;
