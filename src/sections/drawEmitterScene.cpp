...
bool sDrawEmitterScene::load() {
	...
	LOG->Info(LogLevel::LOW, "Draw Emitter Scene [%s]: Num max of particles will be: %d", identifier.c_str(), numMaxParticles);
...
std::string sDrawEmitterScene::debug() {
	std::string msg;
	msg += "[ drawEmitterScene id: " + identifier + " layer:" + std::to_string(layer) + " ]\n";
