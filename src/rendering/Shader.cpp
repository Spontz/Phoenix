...
/*	if (filename.empty())
    ...
    std::string filetext;
    std::string line;
	std::ifstream inFile(ROOT_DIR "data/shaders/" + filename);
...
*/
	std::string s_demopath = DEMO->demoDir;
	std::string s_filepath = s_demopath + "/shaders/" + filename;

	//std::string s_filepath = "C:/Utils/Spontz/Phoenix/data/shaders/" + filename;
