...
Provide optional RTSP monitoring of Phoenix's final OpenGL framebuffer so a running instance can be inspected from VLC without affecting normal rendering when streaming is disabled.
...
#### Scenario: Streaming is toggled repeatedly
- **WHEN** RTSP streaming is disabled and enabled again during the same Phoenix session
- **THEN** the system releases and recreates the listener resources so clients can connect again
...
#### Scenario: Client reconnects after runtime restart
- **WHEN** streaming is stopped from Engine Config and later started again
- **THEN** a VLC client can reconnect to the advertised RTSP URL without restarting Phoenix
...
#### Scenario: Streaming is stopped at runtime
- **WHEN** the user disables RTSP streaming from Engine Config
- **THEN** the system closes any active client, listener, encoder, and queued frame resources without stopping the engine
