...
### Requirement: Deterministic sampling for well-formed animations

For an animation whose channels span the full animation duration, sampling SHALL depend only on the playback time and the keyframe data, so that repeated playback of the same animation always yields the same poses.
...
- **THEN** every sampled pose is the plain interpolation between its bracketing keyframes, with no clamping or hold applied
