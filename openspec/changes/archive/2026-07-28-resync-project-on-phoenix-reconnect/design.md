...
Phoenix may expose multiple internal work units for each section, currently one file-write step and one runtime-load step. Cacablu scales those work units onto the number of sections in the outgoing snapshot before updating the shared progress dialog. The progress bar remains monotonic, but its visible denominator and completed value describe sections rather than Phoenix implementation steps.
...
Before deleting the complete pool, Phoenix deactivates every runtime section, including startup-loaded sections that are absent from the editor dependency index, stops the audio device while clearing cached miniaudio decoders, and clears `VideoManager`. Stopping the audio callback before destroying cached sounds avoids a race and releases Windows file handles held by streamed audio assets. Phoenix restarts the initialized audio device after the cache is empty so replacement sound sections can load normally. Bulk pool writes suppress Phoenix's per-file dependent-section reload. Reconnect synchronization already replaces every section after the complete asset and configuration snapshots are present; reloading an old section while only part of the pool has been restored can observe incomplete dependencies and can reopen media resources during reconstruction. Interactive file-browser writes retain immediate dependent-section reload. Any failed bulk write is propagated with its path and keeps the synchronization generation pending.
