...
namespace Phoenix {
	...
	{
		Logger::info(LogLevel::low, "Video: Loading video {}", videoSource.m_sPath);
		...
		for (int32_t i = 0; i < static_cast<int32_t>(m_pFormatContext->nb_streams); ++i) {
			...
			if (m_bDebug) {
				Logger::info(LogLevel::low, "Video: AVStream->time_base before open coded {}/{}", pAVStream->time_base.num, pAVStream->time_base.den);
				Logger::info(LogLevel::low,	"Video: AVStream->r_frame_rate before open coded {}/{}", pAVStream->r_frame_rate.num, pAVStream->r_frame_rate.den);
				Logger::info(LogLevel::low, "Video: AVStream->start_time {:lld}", pAVStream->start_time);
				Logger::info(LogLevel::low, "Video: AVStream->duration {:lld}", pAVStream->duration);
				Logger::info(LogLevel::low, "Video: finding the proper decoder (CODEC)");
			}
		...
		if (!m_pCodecContext) {
			Logger::error("Video: failed to allocated memory for AVCodecContext");
			return false;
		}
		...
		if (avcodec_parameters_to_context(m_pCodecContext, m_pAVCodecParameters) < 0) {
			Logger::error("Video: failed to copy codec params to codec context");
			return false;
		}

		// Initialize the AVCodecContext to use the given AVCodec.
		if (avcodec_open2(m_pCodecContext, m_pAVCodec, nullptr) < 0) {
			Logger::error("Video: failed to open codec through avcodec_open2");
			return false;
		}

		m_pFrame = av_frame_alloc();
		if (!m_pFrame) {
			Logger::error("Video: failed to allocated memory for m_pFrame");
			return false;
		}

		m_pGLFrame = av_frame_alloc();
		if (!m_pGLFrame) {
			Logger::error("Video: failed to allocated memory for m_pGLFrame");
			return false;
		}
		...
		if (!m_pConvertContext) {
			Logger::error("Video: Could not create the convert context for OpenGL");
			return false;
		}

		m_pAVPacket = av_packet_alloc();
		if (!m_pAVPacket) {
			Logger::error("Video: failed to allocated memory for AVPacket");
			return false;
		}
		...
		while (m_dNextFrameTime <= m_dTime)
		{
			...
			if (av_read_frame(m_pFormatContext, m_pAVPacket) >= 0) {
				...
				if (m_pAVPacket->stream_index == m_VideoSource.m_iVideoStreamIndex)
				{
					if (decodePacket() < 0)
					{
						Logger::error("Packet cannot be decoded, Time: {:.4f}", m_dTime);
					}
	...
	int32_t Video::decodePacket()
	{
		...
		if (iResponse < 0) {
			Logger::error("Video: decodePacket Error while sending a packet to the decoder"); // : %s", av_err2str(response));
			return iResponse;
		}
		...
		while (iResponse >= 0)
		{
			...
			if (iResponse == AVERROR(EAGAIN) || iResponse == AVERROR_EOF)
				...
				Logger::error("Video: Error while receiving a frame from the decoder"); // : %s", av_err2str(response));
				return iResponse;
			}

			if (iResponse >= 0) {
				if (m_bDebug) {
					Logger::info(LogLevel::low,"Frame {} (type={})",m_pCodecContext->frame_num, av_get_picture_type_char(m_pFrame->pict_type));
				}
				...
				{
					//Logger::warning("Video: PTS is invalid or unavailable, using avgFramePeriod.");
