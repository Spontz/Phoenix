...
namespace Phoenix {
	...
	bool sCameraTarget::load()
	{
		// script validation
		if ((param.size() != 1)) {
			Logger::error("Camera Target [{}]: 1 param needed (the camera mode has not been defined)", identifier);
			return false;
		}
		...
		case CameraMode::ONLY_SPLINE:
			if (spline.size() == 0) {
				Logger::error("Camera Target [{}]: Spline mode: a spline file is needed (camera file)", identifier);
				return false;
			}
			break;
		...
		case CameraMode::SPLINE_AND_FORMULA:
			...
			break;
		default:
			Logger::error("Camera Target [{}]: Camera mode not supported", identifier);
			return false;
			break;
		}
